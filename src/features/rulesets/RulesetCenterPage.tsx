...
export function RulesetCenterPage({ rulesetData }: { rulesetData: RulesetData | null }) {
  ...
  return (
    ...
    >
      ...
      <section className="ruleset-roadmap-panel" data-testid="runtime-coverage-certification"><span className="mini-label">Runtime Coverage Certification · %{runtimeCertification.score}</span><h2>{runtimeCertification.status==="certified"?"Mekanik kapsam ölçüldü":"Runtime boşlukları kapatılmalı"}</h2><p>Automatic doğrudan hesaplanır; Assisted sistem yönlendirir; Manual oyuncu/DM tarafından uygulanır; Missing henüz tanımlı değildir.</p><div className="ruleset-roadmap-grid">{runtimeCertification.categories.map(group=><span key={group.id} data-testid={`runtime-coverage-${group.id}`}><strong>{group.label} · %{group.score}</strong><small>{group.total} toplam · {group.automatic} Automatic · {group.assisted} Assisted · {group.manual} Manual · {group.missing} Missing</small></span>)}</div>{runtimeCertification.categories.map(group=><details key={group.id} data-testid={`runtime-coverage-details-${group.id}`}><summary data-testid={`runtime-coverage-summary-${group.id}`}>{group.label} detayları · {group.total}</summary><ul className="ruleset-note-list">{group.entities.map(entity=><li key={entity.id}><strong>{entity.tier.toUpperCase()}</strong> · {entity.name} — {entity.reason}</li>)}</ul></details>)}{runtimeCertification.priorities.length?<details open><summary>Öncelikli runtime açıkları · {runtimeCertification.priorities.length}</summary><ul className="ruleset-note-list">{runtimeCertification.priorities.map(item=><li key={item}>{item}</li>)}</ul></details>:null}</section>
    </PageShell>
  );
}
