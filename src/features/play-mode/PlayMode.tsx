...
export function PlayMode({
  ...
  function castSpell(spellId: string, requestedSlotLevel?:number, consumeSlot=true, inventoryAfterCast?:Character["inventory"]) {
    ...
    const formula=getSpellRollFormula(spell,activeCharacter.level,slotLevel);const total=formula?rollFormula(formula):null;const createdEffect=createSpellEffect(spell);const effectsAfterAction=removeFragileItemEffects(activeSpellEffects);const nextEffects=createdEffect?addSpellEffect(effectsAfterAction,createdEffect):effectsAfterAction;
    commit({
  ...
  function resolvedCheck(label:string,modifier:number,kind:"skill"|"save",ability?:AbilityKey){const imposed=kind==="skill"?exhaustionEffects.abilityCheckMode:exhaustionEffects.attackSaveMode;const potionMode=kind==="save"&&ability==="dex"?itemEffectRuntime.dexSaveMode:"normal";const effectiveMode=combineRollModes(combineRollModes(checkMode,imposed),potionMode);const adjustedModifier=modifier-exhaustionEffects.d20Penalty;const dice=rollDice({count:effectiveMode==="normal"?1:2,sides:20,modifier:0});const natural=chooseD20(dice.rolls,effectiveMode);const potionBonus=kind==="save"&&itemEffectRuntime.attackSaveBonusDice?rollFormula(itemEffectRuntime.attackSaveBonusDice):0;setRollHistory(current=>[{id:dice.id,label,notation:`${effectiveMode} · [${dice.rolls.join(", ")}] ${formatModifier(adjustedModifier)}${potionBonus?` + ${potionBonus} potion`:""}`,total:natural+adjustedModifier+potionBonus},...current].slice(0,6))}
  ...
  function consumeItem(itemId:string){const item=rulesetData?.items.find(candidate=>candidate.id===itemId);if(!item)return;const nextInventory=consumeInventoryItem(activeCharacter.inventory,itemId);const scrollSpell=item.tags?.includes("scroll")?rulesetData?.spells.find(candidate=>candidate.name===item.grantedSpellName):undefined;if(scrollSpell){castSpell(scrollSpell.id,scrollSpell.level,false,nextInventory);return}const formula=getItemHealingFormula(item);let currentHp=activeCharacter.currentHp;if(formula){const total=rollFormula(formula);currentHp=Math.min(activeCharacter.maxHp,currentHp+total);setRollHistory(current=>[{id:crypto.randomUUID(),label:`${item.name} Healing`,notation:formula,total},...current].slice(0,6))}const effect=createItemEffect(item);const itemTempHp=getItemTempHp(item);commit({currentHp,tempHp:Math.max(activeCharacter.tempHp,itemTempHp),inventory:nextInventory,activeSpellEffects:effect?[...activeSpellEffects,effect]:activeSpellEffects})}
