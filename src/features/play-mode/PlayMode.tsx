...
export function PlayMode({
  ...
  function removeManualEffect(id: string, name: string) {
    ...
    setManualEffectRevision(value => value + 1); setActionFeedback(`Manuel etki kaldırıldı: ${name}`);
  }
  ...
  function advanceManualEffects() {
    ...
    setActionFeedback(next.length < manualRuntimeEffects.length ? "Süresi biten manuel etkiler temizlendi" : "Manuel etki süreleri 1 round ilerledi");
  }
  ...
  return (
    ...
    >
      <div className={`play-mode-v1 ${isFocusMode ? "focus-mode" : ""}`}>
        ...
        <section className="manual-runtime-bridge play-mode-card" data-testid="manual-runtime-bridge" aria-label="Manuel runtime köprüsü">
          <div className="play-mode-section-head"><div><span className="mini-label">Playable Gap Closure</span><h2>Manuel Runtime Köprüsü</h2></div><button type="button" disabled={!manualRuntimeEffects.some(effect=>effect.remainingRounds!==null)} onClick={advanceManualEffects}>Round İlerlet</button></div>
          <p>Henüz otomatikleştirilmemiş spell, feat, item veya subclass etkilerini oyun akışını bölmeden burada takip et.</p>
          <div className="manual-runtime-form">
            <label>Etki adı<input value={manualEffectName} onChange={event=>setManualEffectName(event.target.value)} placeholder="Örn. Spirit Shroud" /></label>
            <label>Tür<select value={manualEffectKind} onChange={event=>setManualEffectKind(event.target.value as ManualRuntimeKind)}><option value="spell">Spell</option><option value="feat">Feat</option><option value="item">Item</option><option value="subclass">Subclass</option><option value="class">Class</option><option value="condition">Condition</option><option value="other">Diğer</option></select></label>
            <label>Round<input type="number" min="0" max="999" value={manualEffectRounds} onChange={event=>setManualEffectRounds(Math.max(0,Number(event.target.value)||0))} /><small>0 = süresiz</small></label>
          ...
          <div className="manual-runtime-list" data-testid="manual-runtime-list">{manualRuntimeEffects.length?manualRuntimeEffects.map(effect=><div key={effect.id} className="manual-runtime-entry"><div><strong>{effect.name}</strong><small>{effect.kind}{effect.remainingRounds===null?" · süresiz":` · ${effect.remainingRounds} round`}{effect.note?` · ${effect.note}`:""}</small></div><button type="button" onClick={()=>removeManualEffect(effect.id,effect.name)}>Kaldır</button></div>):<small>Takip edilen manuel etki yok.</small>}</div>
        </section>
