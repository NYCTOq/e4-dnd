...
import { GuidedFeatureControlPanel } from "./GuidedFeatureControlPanel";
...
import { createCombatTurnState, endCombatTurn } from "../../core/rulesets/combatTurnRules";
import { advanceConditionDurations, getCombatStatusLabel, getDeathSaveStatus, shouldRequestConcentrationSave } from "../../core/rulesets/combatStatusRules";
...
import { PlayStatusRuntimePanel } from "./PlayStatusRuntimePanel";
...
export function PlayMode({
  ...
  const [combatTurn,setCombatTurn]=useState(()=>createCombatTurnState(selectedCharacterId||null));
  ...
  const [manualEffectRevision, setManualEffectRevision] = useState(0);
  const [manualEffectName, setManualEffectName] = useState("");
  const [manualEffectKind, setManualEffectKind] = useState<ManualRuntimeKind>("other");
  const [manualEffectRounds, setManualEffectRounds] = useState(0);
  const [manualEffectNote, setManualEffectNote] = useState("");
  ...
  void manualEffectRevision;
  const manualRuntimeEffects = loadManualRuntimeEffects(activeCharacter.id);

  function addManualEffect() {
    const next = addManualRuntimeEffect(manualRuntimeEffects, { name: manualEffectName, kind: manualEffectKind, note: manualEffectNote, rounds: manualEffectRounds || null });
    if (next === manualRuntimeEffects) return;
    saveManualRuntimeEffects(activeCharacter.id, next);
    const label = `Manuel etki eklendi: ${manualEffectName.trim()}`;
    savePlaySession(appendSessionEvent(ensurePlaySession(activeCharacter.id, activeCharacter.name), label, { round: combatTurn.round, turn: combatTurn.turn }));
    setManualEffectName(""); setManualEffectNote(""); setManualEffectRounds(0); setManualEffectRevision(value => value + 1); setSessionRevision(value => value + 1); setActionFeedback(label);
  }

  function removeManualEffect(id: string, name: string) {
    saveManualRuntimeEffects(activeCharacter.id, removeManualRuntimeEffect(manualRuntimeEffects, id));
    setManualEffectRevision(value => value + 1); setActionFeedback(`Manuel etki kaldÄ±rÄ±ldÄ±: ${name}`);
  }

  function advanceManualEffects() {
    const next = advanceManualRuntimeEffects(manualRuntimeEffects);
    saveManualRuntimeEffects(activeCharacter.id, next);
    setManualEffectRevision(value => value + 1);
    setActionFeedback(next.length < manualRuntimeEffects.length ? "SÃ¼resi biten manuel etkiler temizlendi" : "Manuel etki sÃ¼releri 1 round ilerledi");
  }

  function commit(patch: Partial<Character>, label = "Karakter gÃ¼ncellendi") {
    ...
    const nextSession = appendSessionEvent(ensurePlaySession(activeCharacter.id, activeCharacter.name), label, { round: combatTurn.round, turn: combatTurn.turn });
  ...
  function updateHp(amount: number) {
    ...
    }, amount < 0 ? `${Math.abs(amount)} HP azaltÄ±ldÄ±` : `${amount} HP artÄ±rÄ±ldÄ±`);
  }
  ...
  function castSpell(spellId: string, requestedSlotLevel?:number, consumeSlot=true, inventoryAfterCast?:Character["inventory"]) {
    ...
    if (!spell) { setActionFeedback("BÃ¼yÃ¼ verisi bulunamadÄ±."); return; }
  ...
  function consumeItem(itemId:string){
    ...
    if(damageProfile){
      ...
      if(damageProfile.saveAbility&&damageProfile.saveDc){
        ...
        damageRolls.push({id:dice.id,label:`${item.name} Â· ${damageProfile.saveAbility.toUpperCase()} Save Â· ${save.success?"BaÅŸarÄ±lÄ±":"BaÅŸarÄ±sÄ±z"}`,notation:`${spellTargetMode} Â· [${dice.rolls.join(", ")}] ${formatModifier(targetSaveBonus)} vs DC ${damageProfile.saveDc}`,total:save.total});
      }
      damageRolls.push({id:crypto.randomUUID(),label:`${item.name} Â· ${damageProfile.damageType} Damage${targetMatches?"":" Â· Hedef tÃ¼rÃ¼ eÅŸleÅŸmedi"}`,notation:damageProfile.formula,total:resolvedDamage});
  ...
  function spendMagicItemCharge(itemId:string){const item=rulesetData?.items.find(candidate=>candidate.id===itemId);if(!item)return;const cost=item.chargeCost??1;const entry=activeCharacter.inventory.find(candidate=>candidate.itemId===itemId);const state=getItemChargeState(item,entry);if(!state?.ready){setActionFeedback(state?.reason??`${item.name} charge ile kullanÄ±lamÄ±yor.`);return}const nextInventory=spendItemCharge(activeCharacter.inventory,item,cost);if(item.id==="pearl-of-power"){const nextSlots=recoverHighestSpentSpellSlot(spellSlots);if(nextSlots===spellSlots){setActionFeedback("Pearl of Power iÃ§in harcanmÄ±ÅŸ 1-3. seviye slot yok.");return}commit({inventory:nextInventory,spellSlots:nextSlots});setActionFeedback(`${item.name} kullanÄ±ldÄ± Â· ${state.remaining-cost}/${state.maximum} charge kaldÄ±.`);return}commit({inventory:nextInventory});setActionFeedback(`${item.name} kullanÄ±ldÄ± Â· ${state.remaining-cost}/${state.maximum} charge kaldÄ±.`);const spell=rulesetData?.spells.find(candidate=>candidate.name===item.grantedSpellName);if(spell)castSpell(spell.id,spell.level,false)}

  function handleFeatAction(action:FeatAction){const state=getFeatUseState(action,featActionUses);if(!state.unlimited&&state.remaining<=0){setActionFeedback(`${action.name} iÃ§in kullanÄ±m hakkÄ± kalmadÄ±.`);return}if(action.type==="action"&&turnEconomy.actionUsed||action.type==="bonus-action"&&turnEconomy.bonusActionUsed||action.type==="reaction"&&turnEconomy.reactionUsed){setActionFeedback(`${action.name} iÃ§in ${action.type} ekonomisi zaten kullanÄ±ldÄ±.`);return}const tempHp=action.id==="inspiring-leader"?getInspiringLeaderTempHp(activeCharacter.level,getAbilityModifier(effectiveCharacter.abilities.cha)):activeCharacter.tempHp;setFeatActionUses(current=>spendFeatUse(action,current));commit({tempHp:Math.max(activeCharacter.tempHp,tempHp)});setTurnEconomy(current=>spendTurnResource(current,action.type));setActionFeedback(`${action.name} kullanÄ±ldÄ±${state.unlimited?"":` Â· ${state.remaining-1}/${state.maximum} kullanÄ±m kaldÄ±`}.`);setRollHistory(current=>[{id:crypto.randomUUID(),label:`Feat Â· ${action.name}`,notation:action.summary,total:action.id==="inspiring-leader"?tempHp:0},...current].slice(0,6))}

  function handleSubclassAction(action:SubclassRuntimeAction){if(!canUseSubclassAction(action,activeCharacter.resources)){setActionFeedback(`${action.name} iÃ§in kullanÄ±labilir kaynak yok.`);return}if(action.type==="action"&&turnEconomy.actionUsed||action.type==="bonus-action"&&turnEconomy.bonusActionUsed||action.type==="reaction"&&turnEconomy.reactionUsed){setActionFeedback(`${action.name} iÃ§in ${action.type} ekonomisi zaten kullanÄ±ldÄ±.`);return}const resources=spendSubclassActionResource(action,activeCharacter.resources);if(action.id==="tides-of-chaos")setCheckMode("advantage");const damage=action.id==="radiance-of-dawn"?rollDice({count:Math.max(2,Math.ceil(activeCharacter.level/2)),sides:10,modifier:getAbilityModifier(effectiveCharacter.abilities.wis)}):null;const healing=action.id==="preserve-life"?getPreserveLifeHealing(activeCharacter.level,activeCharacter.currentHp,activeCharacter.maxHp):0;commit({resources,currentHp:healing?Math.min(activeCharacter.maxHp,activeCharacter.currentHp+healing):activeCharacter.currentHp});setTurnEconomy(current=>spendTurnResource(current,action.type));const state=getSubclassActionResourceState(action,resources);setActionFeedback(state?`${action.name} kullanÄ±ldÄ± Â· ${state.unlimited?"SÄ±nÄ±rsÄ±z":`${state.remaining}/${state.maximum} kaldÄ±`}`:`${action.name} kullanÄ±ldÄ±.`);setRollHistory(current=>[{id:damage?.id??crypto.randomUUID(),label:`${activeCharacter.subclass} Â· ${action.name}`,notation:damage?.notation??(healing?`+${healing} HP`:action.type),total:damage?.total??healing},...current].slice(0,6))}
  ...
  function weaponAttack(weapon: (typeof equippedItems.weapons)[number]) {
    ...
    const results: RollResult[] = [{ id: dice.id, label: `${weapon.name} Â· ${attack.hit ? attack.critical ? "CRITICAL" : "Hit" : "Miss"}`, notation: `${effectiveMode} Â· [${dice.rolls.join(", ")}] ${formatModifier(modifier)} vs AC ${targetAc}`, total: attack.total }];
    if (attack.hit) {
      ...
      if (formula) {
        ...
        results.push({id:crypto.randomUUID(),label:`${weapon.name} Damage${attack.critical?" Â· Critical":""}${offhandAttack?" Â· Off-hand":""}`,notation:damage.notation,total:damage.total});
        ...
        if(attack.critical&&ancestryRuntime.criticalExtraDice){const savage=rollDice({count:ancestryRuntime.criticalExtraDice,sides:formula.sides,modifier:0});results.push({id:savage.id,label:"Savage Attacks Â· Critical Die",notation:savage.notation,total:savage.total})}
      }
      for (const extra of getMagicWeaponExtraDamage(weapon, magicWeaponTarget, attack.critical)) { const total = rollFormula(extra.formula); results.push({ id: crypto.randomUUID(), label: `${weapon.name} Â· ${extra.damageType} ${extra.source === "critical" ? "Critical " : ""}Damage`, notation: extra.formula, total }); }
      ...
      if (sneakAllowed) { const sneak = rollDice({ count: getSneakAttackDice(activeCharacter.level) * (attack.critical ? 2 : 1), sides: 6, modifier: 0 }); results.push({ id: sneak.id, label: `Sneak Attack${attack.critical ? " Â· Critical" : ""}`, notation: sneak.notation, total: sneak.total }); setSneakAttackUsed(true); }
    }
    if(masteryOutcome.grazeDamage)results.push({id:crypto.randomUUID(),label:`${weapon.name} Â· Graze Damage`,notation:"Ability modifier",total:masteryOutcome.grazeDamage});
    if(masteryOutcome.note)results.push({id:crypto.randomUUID(),label:`Weapon Mastery Â· ${masteryOutcome.mastery}`,notation:masteryOutcome.note,total:0});
  ...
  function unarmedStyleAttack(){
    ...
    setTurnEconomy(current=>spendAttack(current,attacksPerAction));
    setRollHistory(current=>[{id:attack.id,label:`Unarmed Strike Â· ${resolved.hit?resolved.critical?"CRITICAL":"Hit":"Miss"}`,notation:`${attackMode} Â· ${formatModifier(modifier)} vs AC ${targetAc}`,total:resolved.total},...(damage?[{id:damage.id,label:"Unarmed Fighting Damage",notation:damage.notation,total:damage.total}]:[]),...current].slice(0,6));
  }
  function handleDefensiveFightingStyle(style:"protection"|"interception"){
    ...
    setRollHistory(current=>[{id:crypto.randomUUID(),label:style==="protection"?"Protection Â· Ally attack disadvantage":"Interception Â· Damage Reduction",notation:style==="protection"?"Reaction":`1d10 + PB`,total:reduction},...current].slice(0,6));
  }
  ...
  function longRest() {
    ...
    setActionFeedback(["Long Rest tamamlandÄ±; sÄ±nÄ±rlÄ± feat kullanÄ±mlarÄ± yenilendi.",...recoveredChargeSummary].join(" Â· "));
    setLuckyUsed(0);
  }
      ...
      <div className={`play-mode-v1 ${isFocusMode ? "focus-mode" : ""}`}>
        {(actionFeedback || undoSnapshot) ? (
          <section className="play-action-feedback" role="status" aria-live="polite" data-testid="play-action-feedback">
            <div><strong>{actionFeedback ?? "Son oyun iÅŸlemi geri alÄ±nabilir"}</strong><small>{undoSnapshot ? undoSnapshot.label : "Ä°ÅŸlem kaydedildi"}</small></div>
        ...
        <section className="session-play-loop" data-testid="session-play-loop" aria-label="Oyun oturumu Ã¶zeti">
          <div className="session-play-loop-head"><div><span className="mini-label">Session Play Loop</span><h2>{sessionSummary.active ? "Oturum devam ediyor" : "Oturum tamamlandÄ±"}</h2><small>{new Date(playSession.startedAt).toLocaleString("tr-TR")} Â· {sessionSummary.total} kayÄ±t</small></div><div className="session-play-loop-actions">{sessionSummary.active ? <button type="button" data-testid="session-end" onClick={()=>{savePlaySession(endPlaySession(playSession));setSessionRevision(value=>value+1);setActionFeedback("Oyun oturumu tamamlandÄ±")}}>Oturumu Bitir</button> : <button type="button" data-testid="session-new" onClick={()=>{clearPlaySession(activeCharacter.id);ensurePlaySession(activeCharacter.id,activeCharacter.name);setSessionRevision(value=>value+1);setActionFeedback("Yeni oyun oturumu baÅŸlatÄ±ldÄ±")}}>Yeni Oturum</button>}<button type="button" onClick={()=>{clearPlaySession(activeCharacter.id);ensurePlaySession(activeCharacter.id,activeCharacter.name);setSessionRevision(value=>value+1)}}>GeÃ§miÅŸi Temizle</button></div></div>
          ...
          <div className="session-play-loop-events" data-testid="session-event-list">{playSession.events.length ? playSession.events.slice(0,8).map(event=><div className="session-play-loop-event" key={event.id}><div><strong>{event.label}</strong><small>{event.kind} Â· Round {event.round ?? "-"} / Turn {event.turn ?? "-"}</small></div><time>{new Date(event.at).toLocaleTimeString("tr-TR",{hour:"2-digit",minute:"2-digit"})}</time></div>) : <small>Ä°lk aksiyonla birlikte oturum kaydÄ± baÅŸlayacak.</small>}</div>
        </section>

        <section className="manual-runtime-bridge play-mode-card" data-testid="manual-runtime-bridge" aria-label="Manuel runtime kÃ¶prÃ¼sÃ¼">
          <div className="play-mode-section-head"><div><span className="mini-label">Playable Gap Closure</span><h2>Manuel Runtime KÃ¶prÃ¼sÃ¼</h2></div><button type="button" disabled={!manualRuntimeEffects.some(effect=>effect.remainingRounds!==null)} onClick={advanceManualEffects}>Round Ä°lerlet</button></div>
          <p>HenÃ¼z otomatikleÅŸtirilmemiÅŸ spell, feat, item veya subclass etkilerini oyun akÄ±ÅŸÄ±nÄ± bÃ¶lmeden burada takip et.</p>
          <div className="manual-runtime-form">
            <label>Etki adÄ±<input value={manualEffectName} onChange={event=>setManualEffectName(event.target.value)} placeholder="Ã–rn. Spirit Shroud" /></label>
            <label>TÃ¼r<select value={manualEffectKind} onChange={event=>setManualEffectKind(event.target.value as ManualRuntimeKind)}><option value="spell">Spell</option><option value="feat">Feat</option><option value="item">Item</option><option value="subclass">Subclass</option><option value="class">Class</option><option value="condition">Condition</option><option value="other">DiÄŸer</option></select></label>
            <label>Round<input type="number" min="0" max="999" value={manualEffectRounds} onChange={event=>setManualEffectRounds(Math.max(0,Number(event.target.value)||0))} /><small>0 = sÃ¼resiz</small></label>
            <label>Not<input value={manualEffectNote} onChange={event=>setManualEffectNote(event.target.value)} placeholder="Hasar, DC veya masa notu" /></label>
            <button type="button" className="primary-action" disabled={!manualEffectName.trim()} onClick={addManualEffect}>Etkiyi Takibe Al</button>
          </div>
          <div className="manual-runtime-list" data-testid="manual-runtime-list">{manualRuntimeEffects.length?manualRuntimeEffects.map(effect=><div key={effect.id} className="manual-runtime-entry"><div><strong>{effect.name}</strong><small>{effect.kind}{effect.remainingRounds===null?" Â· sÃ¼resiz":` Â· ${effect.remainingRounds} round`}{effect.note?` Â· ${effect.note}`:""}</small></div><button type="button" onClick={()=>removeManualEffect(effect.id,effect.name)}>KaldÄ±r</button></div>):<small>Takip edilen manuel etki yok.</small>}</div>
        </section>
        ...
        <section className="player-journey-integration play-journey-integration" aria-label="Play Mode dinlenme Ã¶zeti"><div><span className="mini-label">Live Journey State</span><strong>{journeySnapshot.restRecommendation==="none"?"Kaynaklar hazÄ±r":journeySnapshot.restRecommendation==="short"?"Short Rest Ã¶neriliyor":"Long Rest Ã¶neriliyor"}</strong><p>{journeySnapshot.restReason}</p></div><div className="player-journey-actions"><NavLink to={`/characters/${activeCharacter.id}`}>Character Sheet</NavLink><NavLink to="/rest">Rest Center</NavLink></div></section>

        <section className={`play-readiness-card ${readiness.status}`}>
          <div>
            <span className="mini-label">Playable Character Check</span>
            <strong>{readiness.status === "ready" ? t("play.ready","Karakter masaya hazÄ±r") : t("play.score",`HazÄ±rlÄ±k skoru: ${readiness.score}%`,{score:readiness.score})}</strong>
            <p>{readiness.status === "ready" ? t("play.readyDetail","HP, progression, seÃ§imler, bÃ¼yÃ¼ler ve ekipman baÄŸlantÄ±larÄ± oynanabilir durumda.") : t("play.errors",`${readiness.issues.filter((issue) => issue.severity === "error").length} zorunlu dÃ¼zeltme var.`,{count:readiness.issues.filter(issue=>issue.severity==="error").length})}</p>
          </div>
          <div className="play-readiness-actions">
            {readiness.issues.filter((issue) => issue.severity === "warning").length ? <span>{readiness.issues.filter((issue) => issue.severity === "warning").length} Ã¶neri</span> : null}
            <NavLink to={`/characters/${activeCharacter.id}/edit`}>{readiness.status === "ready" ? t("play.edit","Karakteri dÃ¼zenle") : t("play.fix","Eksikleri dÃ¼zelt")}</NavLink>
          </div>
        </section>
        ...
        <div className="play-mode-grid">
          ...
          {selectedSubclass?<section className="play-mode-card"><div className="play-mode-section-head"><div><span className="mini-label">Subclass Runtime</span><h2>{selectedSubclass.name}</h2></div><strong>{subclassRuntime.unlockedFeatures.length}/{selectedSubclass.features.length} aÃ§Ä±k</strong></div>{subclassRuntime.actions.length?<div className="play-mode-big-buttons">{subclassRuntime.actions.map(action=>{const resourceState=getSubclassActionResourceState(action,activeCharacter.resources);return <button type="button" key={action.id} data-testid={`subclass-action-${action.id}`} disabled={!canUseSubclassAction(action,activeCharacter.resources)||(action.type==="action"&&turnEconomy.actionUsed)||(action.type==="bonus-action"&&turnEconomy.bonusActionUsed)||(action.type==="reaction"&&turnEconomy.reactionUsed)} onClick={()=>handleSubclassAction(action)}><span>{action.name} Â· {action.type}</span>{resourceState?<small>{resourceState.unlimited?"SÄ±nÄ±rsÄ±z":`${resourceState.remaining}/${resourceState.maximum} kullanÄ±m`}</small>:null}</button>})}</div>:null}<div className="condition-rule-summary">{subclassRuntime.criticalThreshold<20?<small>Critical threshold: {subclassRuntime.criticalThreshold}â€“20</small>:null}{subclassRuntime.attacksPerActionMinimum>1?<small>Extra Attack: {subclassRuntime.attacksPerActionMinimum} attack / Action</small>:null}{subclassRuntime.armorClassFloor?<small>Unarmored AC floor: {subclassRuntime.armorClassFloor}</small>:null}{subclassRuntime.damageResistances.length?<small>Resistance: {subclassRuntime.damageResistances.join(", ")}</small>:null}{subclassRuntime.notes.map(note=><small key={note}>{note}</small>)}</div></section>:null}
          <section className="play-mode-card turn-economy-card"><div className="play-mode-section-head"><div><span className="mini-label">Round {combatTurn.round} Â· Turn {combatTurn.turn}</span><h2>Combat Turn</h2></div><div className="combat-turn-actions"><button type="button" onClick={()=>{setTurnEconomy(createTurnEconomy());setSneakAttackUsed(false);setCombatTurn(current=>({...current,economy:createTurnEconomy()}))}}>Turu Yenile</button><button type="button" onClick={()=>{setTurnEconomy(createTurnEconomy());setSneakAttackUsed(false);setCombatTurn(current=>endCombatTurn({...current,economy:turnEconomy},1));advanceEffectRound()}}>Turu Bitir</button></div></div><div className="combat-turn-summary"><strong>{activeCharacter.name}</strong><span>Movement {Math.max(0,effectiveSpeed-turnEconomy.movementUsed)} ft.</span><span>Concentration DC: max 10 veya hasarÄ±n yarÄ±sÄ±</span></div><div className="turn-resource-grid"><button className={turnEconomy.actionUsed?"spent":""} onClick={()=>setTurnEconomy(current=>spendTurnResource(current,"action"))}>Action <small>{turnEconomy.actionUsed?"KullanÄ±ldÄ±":"HazÄ±r"}</small></button><button className={turnEconomy.bonusActionUsed?"spent":""} onClick={()=>setTurnEconomy(current=>spendTurnResource(current,"bonus-action"))}>Bonus Action <small>{turnEconomy.bonusActionUsed?"KullanÄ±ldÄ±":"HazÄ±r"}</small></button><button className={turnEconomy.reactionUsed?"spent":""} onClick={()=>setTurnEconomy(current=>spendTurnResource(current,"reaction"))}>Reaction <small>{turnEconomy.reactionUsed?"KullanÄ±ldÄ±":"HazÄ±r"}</small></button></div><div className="movement-console"><span>Movement {turnEconomy.movementUsed} / {effectiveSpeed} ft.</span>{[5,10,15].map(feet=><button type="button" key={feet} disabled={turnEconomy.movementUsed>=effectiveSpeed} onClick={()=>setTurnEconomy(current=>spendMovement(current,feet,effectiveSpeed))}>+{feet}</button>)}</div><small>Attack {turnEconomy.attacksUsed} / {attacksPerAction} Â· Spell casting time action kaynaÄŸÄ±nÄ± otomatik harcar.</small></section>
          {activeCharacter.fightingStyleIds?.length?<section className="play-mode-card"><div className="play-mode-section-head"><div><span className="mini-label">Fighting Styles</span><h2>Combat Style Runtime</h2></div><strong>{activeCharacter.fightingStyleIds.length} aktif</strong></div><div className="play-mode-big-buttons">{activeCharacter.fightingStyleIds.includes("unarmed-fighting")?<button type="button" disabled={turnEconomy.actionUsed} onClick={unarmedStyleAttack}>Unarmed Strike</button>:null}{activeCharacter.fightingStyleIds.includes("protection")?<button type="button" disabled={turnEconomy.reactionUsed} onClick={()=>handleDefensiveFightingStyle("protection")}>Protection Â· Reaction</button>:null}{activeCharacter.fightingStyleIds.includes("interception")?<button type="button" disabled={turnEconomy.reactionUsed} onClick={()=>handleDefensiveFightingStyle("interception")}>Interception Â· Reaction</button>:null}</div><div className="condition-rule-summary">{activeCharacter.fightingStyleIds.includes("great-weapon-fighting")?<small>Great Weapon Fighting: 1â€“2 damage die bir kez reroll.</small>:null}{activeCharacter.fightingStyleIds.includes("two-weapon-fighting")?<small>Two-Weapon Fighting: off-hand ability damage korunur.</small>:null}{activeCharacter.fightingStyleIds.includes("blind-fighting")?<small>Blind Fighting: 10 ft. blindsight.</small>:null}</div></section>:null}
          {isRogue?<section className="play-mode-card"><div className="play-mode-section-head"><div><span className="mini-label">Rogue Combat</span><h2>Sneak Attack Â· {getSneakAttackDice(activeCharacter.level)}d6</h2></div><strong>{sneakAttackUsed?"Bu tur kullanÄ±ldÄ±":"HazÄ±r"}</strong></div><div className="rogue-toggle-grid"><label><input type="checkbox" checked={sneakAttackEnabled} onChange={event=>setSneakAttackEnabled(event.target.checked)}/> Ä°sabette uygula</label><label><input type="checkbox" checked={sneakAllyAdjacent} onChange={event=>setSneakAllyAdjacent(event.target.checked)}/> Hedefin yanÄ±nda mÃ¼ttefik var</label></div>{rogueFeatures.cunningAction?<div className="play-mode-big-buttons">{["Dash","Disengage","Hide"].map(action=><button type="button" key={action} disabled={turnEconomy.bonusActionUsed} onClick={()=>setTurnEconomy(current=>spendTurnResource(current,"bonus-action"))}>{action}</button>)}</div>:null}<div className="condition-rule-summary">{rogueFeatures.uncannyDodge?<small>Uncanny Dodge: gÃ¶rÃ¼len saldÄ±rÄ±ya Reaction ile yarÄ±m hasar.</small>:null}{rogueFeatures.evasion?<small>Evasion: DEX save hasarÄ± baÅŸarÄ±da 0, baÅŸarÄ±sÄ±zlÄ±kta yarÄ±m.</small>:null}{rogueFeatures.reliableTalent?<small>Reliable Talent: proficient check d20 sonucu minimum 10.</small>:null}{rogueFeatures.elusive?<small>Elusive: incapacitated deÄŸilken saldÄ±rÄ±lar sana advantage alamaz.</small>:null}</div></section>:null}
          {isBarbarian?<section className="play-mode-card"><div className="play-mode-section-head"><div><span className="mini-label">Barbarian Combat</span><h2>Rage Â· +{getRageDamageBonus(activeCharacter.level)} damage</h2></div><strong>{isRaging?"Raging":"HazÄ±r deÄŸil"}</strong></div><div className="rogue-toggle-grid"><label><input type="checkbox" checked={recklessAttack} disabled={!barbarianFeatures.recklessAttack} onChange={event=>setRecklessAttack(event.target.checked)}/> Reckless Attack</label>{activeCharacter.ruleset==="dnd_2024"?<span>Brutal Strike {getBrutalStrike(activeCharacter.level,"dnd_2024").dice}d10 Â· {getBrutalStrike(activeCharacter.level,"dnd_2024").effectCount} effect</span>:<span>Brutal Critical +{getBrutalCriticalExtraDice(activeCharacter.level,activeCharacter.ruleset)} die</span>}</div><div className="condition-rule-summary"><small>STR melee uygun silah: {equippedItems.weapons.some((weapon)=>isRageDamageWeapon(weapon,activeCharacter.ruleset==="dnd_2024"?"dnd_2024":"dnd_2014","str"))?"var":"yok"}</small>{barbarianFeatures.dangerSense?<small>Danger Sense: DEX save advantage.</small>:null}{barbarianFeatures.relentlessRage?<small>Relentless Rage: baÅŸarÄ±lÄ± CON save sonrasÄ± ${getRelentlessRageRecoveryHp(activeCharacter.level,activeCharacter.ruleset==="dnd_2024"?"dnd_2024":"dnd_2014")} HP.</small>:null}{barbarianFeatures.persistentRage?<small>Persistent Rage etkin.</small>:null}</div></section>:null}
          {isMonk?<section className="play-mode-card"><div className="play-mode-section-head"><div><span className="mini-label">Monk Combat</span><h2>Martial Arts Â· d{getMartialArtsDie(activeCharacter.level,activeCharacter.ruleset)}</h2></div><strong>{focusPoints?`${focusPoints.max-focusPoints.used} / ${focusPoints.max} ${activeCharacter.ruleset==="dnd_2024"?"Focus":"Ki"}`:"Kaynak yok"}</strong></div><div className="play-mode-big-buttons"><button type="button" disabled={turnEconomy.actionUsed} onClick={()=>monkAction("Unarmed Strike",0,"action")}>Unarmed Strike</button>{monkFeatures.flurry?<button type="button" disabled={turnEconomy.bonusActionUsed||!focusPoints||focusPoints.used>=focusPoints.max} onClick={()=>monkAction("Flurry of Blows",1,"bonus-action")}>Flurry Â· 1</button>:null}{monkFeatures.patientDefense?<button type="button" disabled={turnEconomy.bonusActionUsed||!focusPoints||focusPoints.used>=focusPoints.max} onClick={()=>monkAction("Patient Defense",1,"bonus-action")}>Patient Defense Â· 1</button>:null}{monkFeatures.stepOfWind?<button type="button" disabled={turnEconomy.bonusActionUsed||!focusPoints||focusPoints.used>=focusPoints.max} onClick={()=>monkAction("Step of the Wind",1,"bonus-action")}>Step of Wind Â· 1</button>:null}</div><div className="condition-rule-summary"><small>Unarmored Movement +{getUnarmoredMovementBonus(activeCharacter.level)} ft.</small>{monkFeatures.deflectAttacks?<small>Deflect Attacks aktif.</small>:null}{monkFeatures.stunningStrike?<small>Stunning Strike: isabette 1 Focus/Ki.</small>:null}{monkFeatures.evasion?<small>Evasion aktif.</small>:null}{monkFeatures.diamondSoul?<small>Diamond Soul / Disciplined Survivor aktif.</small>:null}</div></section>:null}
          {isBard?<section className="play-mode-card"><div className="play-mode-section-head"><div><span className="mini-label">Bard Support</span><h2>Bardic Inspiration Â· d{getBardicInspirationDie(activeCharacter.level)}</h2></div><strong>{bardicInspiration?`${bardicInspiration.max-bardicInspiration.used} / ${bardicInspiration.max} kaldÄ±`:"Kaynak yok"}</strong></div><button type="button" disabled={!bardicInspiration||bardicInspiration.used>=bardicInspiration.max||turnEconomy.bonusActionUsed} onClick={grantBardicInspiration}>Inspiration Ver Â· Bonus Action</button><div className="condition-rule-summary"><small>Recovery: {getInspirationRecovery(activeCharacter.level)} rest</small>{bardFeatures.songOfRest?<small>Song of Rest +d{bardFeatures.songOfRest}</small>:null}{bardFeatures.fontOfInspiration?<small>Font of Inspiration aktif.</small>:null}{bardFeatures.countercharm?<small>Countercharm aktif.</small>:null}{bardFeatures.magicalSecrets?<small>Magical Secrets aktif.</small>:null}{bardFeatures.superiorInspiration?<small>Superior Inspiration aktif.</small>:null}</div></section>:null}
          ...
          <section className="play-mode-card play-mode-hp-card">
            ...
            <div className="survival-console">
              ...
              <select aria-label="Gelen hasar tÃ¼rÃ¼" value={incomingDamageType} onChange={event=>setIncomingDamageType(event.target.value)}><option value="physical">Physical</option><option value="fire">Fire</option><option value="cold">Cold</option><option value="lightning">Lightning</option><option value="poison">Poison</option><option value="acid">Acid</option><option value="necrotic">Necrotic</option><option value="radiant">Radiant</option><option value="psychic">Psychic</option><option value="force">Force</option><option value="thunder">Thunder</option></select>
              ...
              <button data-testid="death-dying-heal" type="button" onClick={healDamage}>Ä°yileÅŸtir</button>
            </div>
            ...
            {itemEffectRuntime.damageResistance?<label className="rage-resistance-toggle"><input type="checkbox" checked={incomingPotionResisted} onChange={event=>setIncomingPotionResisted(event.target.checked)}/> Gelen hasar potion direnciyle eÅŸleÅŸiyor</label>:null}
            ...
            {magicArmorRuntime.preventsCriticalDamage?<small>Adamantine: gelen critical hit normal hit sayÄ±lÄ±r.</small>:null}
            {pendingConcentrationDc?<button type="button" className="concentration-prompt" onClick={()=>concentrationSave(pendingConcentrationDc)}>Concentration Save At Â· DC {pendingConcentrationDc}</button>:null}
            {activeCharacter.currentHp===0?<div data-testid="death-dying-console" className="death-save-console"><strong data-testid="death-dying-status">{getCombatStatusLabel(deathSaveStatus)} Â· âœ“ {activeCharacter.deathSaves.successes}/3 Â· âœ• {activeCharacter.deathSaves.failures}/3</strong><button data-testid="death-dying-roll" type="button" disabled={deathSaveStatus!=="dying"} onClick={rollDeathSave}>Death Save At</button><button data-testid="death-dying-stabilize" type="button" disabled={deathSaveStatus!=="dying"} onClick={stabilize}>Stabilize Et</button>{deathSaveStatus==="stable"?<small>Karakter stabil, yeni hasar almadÄ±kÃ§a death save atmaz.</small>:null}{deathSaveStatus==="dead"?<small>3 baÅŸarÄ±sÄ±zlÄ±k: karakter Ã¶ldÃ¼.</small>:null}</div>:null}
          ...
          <section className="play-mode-card">
            ...
            <div className="exhaustion-console"><div><span className="mini-label">Exhaustion</span><strong>{exhaustionEffects.level} / 6</strong><small>{activeCharacter.ruleset==="dnd_2024"?`D20 -${exhaustionEffects.d20Penalty} Â· Speed ${effectiveSpeed} ft.`:`Speed ${effectiveSpeed} ft. Â· Effective Max HP ${effectiveMaxHp}`}</small>{exhaustionEffects.dead?<em>Level 6 Â· Ã–lÃ¼</em>:null}</div><div><button type="button" disabled={exhaustionEffects.level<=0} onClick={()=>commit({exhaustion:exhaustionEffects.level-1})}>âˆ’</button><button type="button" disabled={exhaustionEffects.level>=6} onClick={()=>commit({exhaustion:exhaustionEffects.level+1})}>+</button></div></div>
          </section>
          ...
          {wildShapeForms.length ? <section className="play-mode-card"><div className="play-mode-section-head"><div><span className="mini-label">Druid</span><h2>Wild Shape</h2></div><strong>{wildShapeResource?`${wildShapeResource.max-wildShapeResource.used} / ${wildShapeResource.max} kullanÄ±m`:"Kaynak yok"}</strong></div><div className="play-mode-slot-grid">{wildShapeForms.map(form=><div className="play-mode-slot-row" key={form.id}><div><span>{form.name} Â· CR {form.challengeRating}</span><small>AC {form.armorClass} Â· HP {form.hitPoints} Â· {form.movement}</small></div><button type="button" disabled={!wildShapeResource||wildShapeResource.used>=wildShapeResource.max} onClick={useWildShape}>DÃ¶nÃ¼ÅŸ</button></div>)}</div></section>:null}

          {maneuvers.length?<section className="play-mode-card"><div className="play-mode-section-head"><div><span className="mini-label">Battle Master Â· {getSuperiorityDie(activeCharacter.level)}</span><h2>Maneuvers</h2></div><strong>{superiorityDice?`${superiorityDice.max-superiorityDice.used} / ${superiorityDice.max} zar`:"Kaynak yok"}</strong></div><div className="play-mode-slot-grid">{maneuvers.map(option=><div className="play-mode-slot-row" key={option.id}><div><span>{option.name}</span><small>{option.trigger} Â· {option.summary}</small></div><button type="button" disabled={!superiorityDice||superiorityDice.used>=superiorityDice.max} onClick={useSuperiorityDie}>Zar Harca</button></div>)}</div></section>:null}

          {companion&&companionStats?<section className="play-mode-card"><div className="play-mode-section-head"><div><span className="mini-label">Beast Master Companion</span><h2>{companion.name}</h2></div><strong>{companionCurrentHp} / {companionStats.maxHp} HP</strong></div><p>{companion.summary}</p><div className="play-mode-core-stats"><div><span>AC</span><strong>{companionStats.armorClass}</strong></div><div><span>Attack</span><strong>{formatModifier(companionStats.attackBonus)}</strong></div><div><span>Damage</span><strong>{companionStats.damage}</strong></div></div><div className="play-mode-big-buttons">{[-5,-1,1,5].map(amount=><button key={amount} onClick={()=>updateCompanionHp(amount)}>{amount>0?`+${amount}`:amount}</button>)}</div><small>{companion.attackName} Â· {companion.speed}</small></section>:null}

          {classActions.length?<section className="play-mode-card"><div className="play-mode-section-head"><div><span className="mini-label">Core Class Actions</span><h2>{activeCharacter.className} KaynaklarÄ±</h2></div></div><div className="play-mode-slot-grid">{classActions.map(action=>{const resourceId=action.resourceId!;const resource=activeCharacter.resources.find(item=>item.id===resourceId)!;const remaining=resource.unlimited?Number.POSITIVE_INFINITY:resource.max-resource.used;return <div className="play-mode-slot-row" key={action.id}><div><span>{action.name} Â· {action.actionType}</span><small>{action.summary}</small><strong>{resource.unlimited?"âˆ kullanÄ±m":`${remaining} / ${resource.max} kaldÄ±`} Â· {resource.shortRecoveryAmount?`Short Rest +${resource.shortRecoveryAmount}`:`${resource.recovery} rest`}</strong></div>{resourceId==="lay-on-hands"?<div>{[1,5,10].map(amount=><button key={amount} disabled={remaining<amount} onClick={()=>executeClassAction(resourceId,amount)}>+{amount} HP</button>)}</div>:<button type="button" disabled={remaining<1} onClick={()=>executeClassAction(resourceId)}>{resourceId==="second-wind"?"Ä°yileÅŸ":"Kullan"}</button>}</div>})}</div></section>:null}

          {arcanumSpells.length?<section className="play-mode-card"><div className="play-mode-section-head"><div><span className="mini-label">Warlock Â· Long Rest</span><h2>Mystic Arcanum</h2></div><strong>{arcanumSpells.length-(activeCharacter.usedArcanumSpellIds??[]).length} hazÄ±r</strong></div><div className="play-mode-spell-list">{arcanumSpells.sort((a,b)=>a.level-b.level).map(spell=>{const used=activeCharacter.usedArcanumSpellIds?.includes(spell.id);return <button key={spell.id} disabled={used} onClick={()=>castArcanum(spell.id)}><span>Level {spell.level} Â· {spell.name}</span><small>{used?"KullanÄ±ldÄ±":"Arcanum Kullan"}</small></button>})}</div></section>:null}

          {isPaladin(activeCharacter.className)&&activeCharacter.level>=2?<section className="play-mode-card"><div className="play-mode-section-head"><div><span className="mini-label">Paladin Combat</span><h2>Divine Smite</h2></div><label><input type="checkbox" checked={smiteHolyTarget} onChange={event=>setSmiteHolyTarget(event.target.checked)}/> Fiend / Undead</label></div><div className="play-mode-slot-grid">{spellSlots.map(slot=>{const dice=getDivineSmiteDice(slot.level,activeCharacter.ruleset,smiteHolyTarget);return <div className="play-mode-slot-row" key={slot.level}><div><span>Level {slot.level} Slot Â· {dice}d8 radiant</span><small>{slot.max-slot.used} / {slot.max} slot kaldÄ±</small></div><button type="button" disabled={slot.used>=slot.max} onClick={()=>divineSmite(slot.level)}>Smite At</button></div>})}</div>{paladinAuras?<div className="play-mode-slot-grid">{paladinAuras.protection?<div className="play-mode-slot-row"><div><span>Aura of Protection Â· {paladinAuras.protection.radius} ft.</span><small>{paladinAuras.protection.summary}</small></div></div>:null}{paladinAuras.courage?<div className="play-mode-slot-row"><div><span>Aura of Courage Â· {paladinAuras.courage.radius} ft.</span><small>{paladinAuras.courage.summary}</small></div></div>:null}{paladinAuras.radiantStrikes?<div className="play-mode-slot-row"><div><span>Radiant Strikes</span><small>{paladinAuras.radiantStrikes}</small></div></div>:null}</div>:null}</section>:null}

          {activeSpellEffects.length?<section className="play-mode-card"><div className="play-mode-section-head"><div><span className="mini-label">Persistent Magic</span><h2>Aktif Spell Etkileri</h2></div><button type="button" onClick={advanceEffectRound}>1 Round Ä°lerle</button></div><div className="play-mode-slot-grid">{activeSpellEffects.map(effect=><div className="play-mode-slot-row" key={effect.id}><div><span>{effect.name}{effect.concentration?" Â· Concentration":""}</span><small>{effect.remainingRounds===null?"SÃ¼resiz":`${effect.remainingRounds} round kaldÄ±`} Â· {effect.conditions?.length?`${effect.conditions.join(", ")} Â· `:""}{effect.summary}</small></div><button type="button" onClick={()=>endSpellEffect(effect.id)}>Bitir</button></div>)}</div>{activeSpellEffects.some(effect=>effect.concentration)?<div className="play-mode-big-buttons"><span>Concentration save:</span>{[10,15,20].map(dc=><button type="button" key={dc} onClick={()=>concentrationSave(dc)}>DC {dc}</button>)}</div>:null}</section>:null}
          ...
          <section className="play-mode-card">
            ...
            <div className="attack-control-bar">
              ...
              <label>AtÄ±ÅŸ<select value={attackMode} onChange={event=>setAttackMode(event.target.value as RollMode)}><option value="normal">Normal</option><option value="advantage">Advantage</option><option value="disadvantage">Disadvantage</option></select></label>
              <label>Hedef tÃ¼rÃ¼<select value={magicWeaponTarget} onChange={event=>setMagicWeaponTarget(event.target.value as MagicWeaponTarget)}><option value="normal">Normal</option><option value="dragon">Dragon</option><option value="giant">Giant</option><option value="undead-fiend">Undead / Fiend</option></select></label>
              {masteryVexReady?<strong>Vex Advantage hazÄ±r</strong>:null}
            </div>
          ...
          <section className="play-mode-card play-mode-spell-card">
            ...
            <div className="spell-target-console"><label>Hedef modu<select value={spellTargetMode} onChange={event=>setSpellTargetMode(event.target.value as RollMode)}><option value="normal">Normal</option><option value="advantage">Advantage</option><option value="disadvantage">Disadvantage</option></select></label><label>Hedef save bonus<input type="number" value={targetSaveBonus} onChange={event=>setTargetSaveBonus(Number(event.target.value)||0)}/></label><label>BaÅŸarÄ±lÄ± save<select value={saveDamageRule} onChange={event=>setSaveDamageRule(event.target.value as "auto"|SaveDamageRule)}><option value="auto">BÃ¼yÃ¼ kuralÄ±</option><option value="half">YarÄ±m hasar</option><option value="none">Hasar yok</option></select></label><small>Spell attack bÃ¼yÃ¼leri Quick Rolls bÃ¶lÃ¼mÃ¼ndeki hedef AC deÄŸerini kullanÄ±r.</small></div>

            <label className="rage-resistance-toggle">Etki alanÄ±ndaki hedef sayÄ±sÄ±<input type="number" min="1" max="20" value={spellTargetCount} onChange={event=>setSpellTargetCount(Math.max(1,Math.min(20,Number(event.target.value)||1)))}/></label>
          ...
          {(homebrewContentRuntime.feats.length||homebrewContentRuntime.spells.length||homebrewContentRuntime.items.length) ? <section className="play-mode-card">
            ...
            {homebrewContentRuntime.feats.length?<div className="play-mode-roll-history">{homebrewContentRuntime.feats.map(entry=><div key={entry.entityId}><span>{entry.feat.name}<small>{entry.passiveSummaries.join(" Â· ")}</small></span><strong>{entry.choiceComplete?"Aktif":"SeÃ§im eksik"}</strong></div>)}</div>:null}
            {homebrewContentRuntime.spells.length?<div className="play-mode-roll-history">{homebrewContentRuntime.spells.map(entry=><div key={entry.entityId}><span>{entry.spell.name}<small>{[entry.formula,entry.spell.effectType,entry.prepared?"Prepared":entry.known?"Known":null,...entry.warnings].filter(Boolean).join(" Â· ")}</small></span><strong>{entry.automatic?"Automatic":"Guided"}</strong></div>)}</div>:null}
            {homebrewContentRuntime.items.length?<div className="play-mode-roll-history">{homebrewContentRuntime.items.map(entry=><div key={entry.entityId}><span>{entry.item.name}<small>{[...entry.effectSummary,entry.chargesRemaining!==undefined?`${entry.chargesRemaining} charge`:null,entry.item.requiresAttunement?(entry.attuned?"Attuned":"Attunement gerekli"):null].filter(Boolean).join(" Â· ")}</small></span><strong>{entry.usable?"KullanÄ±labilir":"HazÄ±r deÄŸil"}</strong></div>)}</div>:null}
          </section>:null}

          {homebrewRuntime.entities.length ? <section className="play-mode-card">
            <div className="play-mode-section-head"><div><span className="mini-label">Homebrew Runtime</span><h2>Ã–zel Ã–zellikler</h2></div>{homebrewRuntime.needsResourceSync?<button type="button" onClick={syncHomebrewResources}>KaynaklarÄ± Senkronize Et</button>:null}</div>
            {homebrewRuntime.progressionFeatures.length?<div className="play-mode-roll-history">{homebrewRuntime.progressionFeatures.map(feature=><div key={`${feature.entityId}-${feature.level}-${feature.name}`}><span>{feature.name}<small>{feature.entityName} Â· Level {feature.level}</small></span><strong>Aktif</strong></div>)}</div>:null}
            <div className="play-mode-class-actions">{homebrewRuntime.actions.filter(item=>item.action.economy!=="passive").map(item=><button type="button" key={item.id} disabled={!item.available} onClick={()=>executeHomebrewAction(item.id)}><span>{item.action.name}</span><small>{item.entityName} Â· {item.action.economy}{item.resourceId?` Â· ${item.action.resourceCost??0} kaynak`:""}</small>{item.reason?<small>{item.reason}</small>:null}</button>)}</div>
          </section>:null}

          <PlayStatusRuntimePanel character={activeCharacter} onChange={onUpdateCharacter} />


          <GuidedFeatureControlPanel characterId={activeCharacter.id} />
