  ...
  return (
    <PageShell eyebrow="Character Foundation" title="Classes + Level Tables" description="2014 ve 2024 class progression verilerini edition bazında incele. Hit Die, save proficiency, subclass seviyesi, spell slotları ve level feature akışı artık ayrı veri katmanlarından geliyor.">
      ...
      <section className="class-catalog-toolbar">
        ...
        <input data-testid="class-catalog-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Class ara..." aria-label="Class ara" />
      </section>
      <div className="class-catalog-layout">
        <aside className="class-catalog-list" aria-label="Class listesi">
          {classes.map((item) => <button data-testid={`class-catalog-option-${item.id}`} type="button" key={item.id} className={selected?.id === item.id ? "active" : ""} onClick={() => setSelectedId(item.id)}><strong>{item.name}</strong><span>d{item.hitDie} · Subclass L{item.subclassLevel}</span></button>)}
        </aside>
        {selected ? <section className="class-catalog-detail" data-testid="class-catalog-detail">
          ...
          <div className="class-level-table-wrap"><table className="class-level-table" data-testid="class-level-table"><thead><tr><th>Level</th><th>PB</th><th>Features</th><th>Spell slots / Pact</th><th>Mastery</th></tr></thead><tbody>{selected.levels.map((row) => <tr data-testid={`class-level-${row.level}`} key={row.level}><td>{row.level}</td><td>+{row.proficiencyBonus}</td><td>{row.features.join(", ") || "—"}</td><td>{row.pactMagic ? `${row.pactMagic.slots} × L${row.pactMagic.slotLevel}` : formatSpellSlots(row.spellSlots)}</td><td>{row.weaponMasteryCount ?? "—"}</td></tr>)}</tbody></table></div>
