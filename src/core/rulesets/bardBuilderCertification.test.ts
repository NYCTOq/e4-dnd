...
describe("Bard full builder certification",()=>{
 it("certifies every 2014 race × subclass × level combination",()=>{const result=rows("2014");expect(result).toHaveLength(9*8*20);expect(summarizeBardCertification(result)).toMatchObject({ready:true,scenarioCount:1440,blockerCount:0})});
 it("certifies every 2024 species × subclass × level combination",()=>{const result=rows("2024");expect(result).toHaveLength(10*4*20);expect(summarizeBardCertification(result)).toMatchObject({ready:true,scenarioCount:800,blockerCount:0})});
 ...
 it("contains complete official Bard subclass feature checkpoints",()=>{for(const edition of ["2014","2024"] as const){const subs=merged(edition).filter(x=>x.className==="Bard");expect(subs).toHaveLength(edition==="2014"?8:4);for(const sub of subs)expect([...new Set(sub.features.map(x=>x.level))]).toEqual([3,6,14])}});
});
