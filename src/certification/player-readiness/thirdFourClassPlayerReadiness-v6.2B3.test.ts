...
type TargetClass = "Paladin" | "Ranger" | "Sorcerer" | "Warlock";
...
type ReadyEvidence = {
  ...
  spellCount: number;
  spellLevelDistribution: Record<string, number>;
  featCatalogCount: number;
  levelsFound: number[];
  featureLevels: number[];
  blockers: string[];
  warnings: string[];
...
type ClassEvidence = ReadyEvidence | BlockedEvidence;
...
function hasClassFeature(
  ...
  return classData.levels.some((level) =>
    level.features.some((feature) =>
      featureName(feature).toLowerCase().includes(normalized),
    ),
  );
}

function buildRulesetEvidence(ruleset: RulesetId): ClassEvidence[] {
  ...
  return TARGET_CLASSES.map((className): ClassEvidence => {
    ...
    if (classData.spellProgression === "none") {
      blockers.push("Expected a spellcasting progression");
    }
    ...
    const signatureFeatures: Record<TargetClass, string[]> = {
      Paladin: ["lay on hands", "aura of protection"],
      Ranger: ["fighting style", "extra attack"],
      Sorcerer: ["spellcasting", "metamagic"],
      Warlock: ["pact magic", "eldritch invocations"],
    };

    for (const signature of signatureFeatures[className]) {
      if (!hasClassFeature(classData, signature)) {
        blockers.push(`Missing signature feature: ${signature}`);
      }
    }
    ...
    if (className === "Warlock") {
      if (classData.spellProgression !== "pact") {
        warnings.push(
          `Warlock spell progression is ${classData.spellProgression}, expected pact`,
        );
      }

      if (!hasClassFeature(classData, "mystic arcanum")) {
        warnings.push("Mystic Arcanum feature was not detected");
      }
    }

    if (className === "Paladin" || className === "Ranger") {
      const maxCatalogLevel = Math.max(
        0,
        ...classSpells.map((entry) => entry.level),
      );

      if (maxCatalogLevel < 5) {
        warnings.push(
          `Highest catalog spell level is ${maxCatalogLevel}; expected half-caster access through level 5`,
        );
      }
    }
...
function writeReports(evidence: ClassEvidence[][]) {
  ...
  fs.writeFileSync(
    path.join(
      reportsDir,
      "THIRD_FOUR_CLASS_PLAYER_READINESS_v6.2B3.json",
    ),
  ...
  const lines = [
    "# E4 D&D Third Four-Class Player Readiness v6.2B3",
    "",
  ...
  for (const entry of flattened) {
    ...
    if (entry.status === "ready") {
      lines.push(
        ...
        `- Spell progression: ${entry.spellProgression}`,
      );
    }
  ...
  fs.writeFileSync(
    path.join(
      reportsDir,
      "THIRD_FOUR_CLASS_PLAYER_READINESS_v6.2B3.md",
    ),
    lines.join("\n"),
    "utf8",
  );
...
describe("v6.2B3 third four-class player readiness", () => {
  const evidence = [
    ...
    for (const entry of rulesetEvidence) {
      it(`${entry.ruleset} ${entry.className} supports a complete 1-20 player build foundation`, () => {
        ...
        if (entry.status === "ready") {
          ...
          expect(entry.spellCount).toBeGreaterThan(0);
        }
      });
    }
  }
