...
type TargetClass = "Barbarian" | "Bard" | "Druid" | "Monk";
...
function buildRulesetEvidence(ruleset: RulesetId): ClassEvidence[] {
  ...
  return TARGET_CLASSES.map((className): ClassEvidence => {
    ...
    const signatureFeatures: Record<TargetClass, string[]> = {
      ...
      Bard: ["bardic inspiration", "expertise"],
    ...
    if (className === "Bard" || className === "Druid") {
...
function writeReports(evidence: ClassEvidence[][]) {
  ...
  fs.writeFileSync(
    path.join(
      reportsDir,
      "SECOND_FOUR_CLASS_PLAYER_READINESS_v6.2B2.json",
    ),
  ...
  const lines = [
    "# E4 D&D Second Four-Class Player Readiness v6.2B2",
    "",
  ...
  fs.writeFileSync(
    path.join(
      reportsDir,
      "SECOND_FOUR_CLASS_PLAYER_READINESS_v6.2B2.md",
    ),
    lines.join("\n"),
    "utf8",
  );
...
describe("v6.2B2 second four-class player readiness", () => {
  const evidence = [
