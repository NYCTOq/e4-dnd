...
# Compatible recursive verification for Windows PowerShell 5.1.
$stale = Get-ChildItem $E2eRoot -Recurse -File -Include *.ts,*.tsx |
  Select-String -Pattern 'e4_dnd_last_seen_version_v1.*6\.1\.0|__E4_E2E_APP_VERSION__\s*=\s*["'']6\.1\.0'
...
$code = $LASTEXITCODE
if ($code -ne 0) {
  Write-Host "N-MEGA12 HOTFIX12 RED (exit=$code)"
  exit $code
}

Write-Host 'N-MEGA12 HOTFIX12 BUILDER CATALOG GREEN'
