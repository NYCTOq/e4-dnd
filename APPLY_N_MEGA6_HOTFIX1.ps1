...
Set-Location $Root
$AuditSource = Join-Path $Root "n-mega6-subclass-feature-runtime.mjs"
$AuditTarget = Join-Path $Root "n-mega6-subclass-feature-runtime.mjs"
if (-not (Test-Path $AuditSource)) { throw "N-MEGA6 hotfix audit dosyasi bulunamadi: $AuditSource" }
Write-Host "N-MEGA6 HOTFIX1 audit basliyor..."
& node $AuditTarget
if ($LASTEXITCODE -ne 0) { throw "N-MEGA6 HOTFIX1 audit RED" }
$Tests = @(
...
if ($LASTEXITCODE -ne 0) { throw "N-MEGA6 HOTFIX1 regression RED" }
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "N-MEGA6 HOTFIX1 build RED" }
Write-Host "N-MEGA6 HOTFIX1 GREEN"
